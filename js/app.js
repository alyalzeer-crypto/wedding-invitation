...
async function submitRsvp(event) {
  ...
  const netlifySite = window.location.hostname.endsWith(".netlify.app") ||
    document.documentElement.dataset.rsvpProvider === "netlify";
  if (window.location.protocol === "file:") {
    rsvpStatus.dataset.state = "error";
    rsvpStatus.textContent = "لن يصل الرد عند فتح الملف مباشرة. افتح الدعوة من رابط الموقع.";
    return;
  }
  if (!localServer && !netlifySite) {
    rsvpStatus.dataset.state = "error";
    rsvpStatus.textContent = "لم يُفعّل استقبال الردود بعد. سيتم ربط النموذج بخدمة استقبال بعد النشر.";
    return;
  }
